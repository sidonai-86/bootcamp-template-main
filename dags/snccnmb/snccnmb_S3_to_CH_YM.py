...
def check_new_files(**context):
    ...
    Variable.set("LAST_S3_CHECK_TIME_YANDEX", now_str)

    manifest_content = "\n".join(path.strip() for path in new_files if path.strip())
    manifest_key = "snccnmb/api_yandex_metrika/manifests/new_files.txt"
...
dag = DAG(
    dag_id="snccnmb_S3_to_CH_YM",
    default_args=default_args,
    schedule_interval="0 16 * * *",
    catchup=False,
    description="S3 yandex to CH",
    tags=["yandex", "metrika", "visits", "hits"],
)
...
s3_to_ch = SparkSubmitOperator(
    ...
    application="/opt/airflow/scripts/transform/transform_YM_snccnmb.py",
    ...
    env_vars={
        ...
        "TABLE_NAME": "ym_from_s3",
        "S3_PATH_MANIFEST": "s3a://dev/{{ ti.xcom_pull(task_ids='check_new_files', key='manifest_key') }}",
        "S3_PATH_YANDEX_METRIKA": "s3a://dev/snccnmb/api_yandex_metrika",
