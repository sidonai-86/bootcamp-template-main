...
def check_new_files(**context):
    ...
    Variable.set("Denis_LAST_S3_CHECK_TIME__WEATHER", now_str)

    manifest_content = "\n".join(path.strip() for path in new_files if path.strip())
    manifest_key = "Denis_Chinese/Weather/manifests/new_files.txt"
...
dag = DAG(
    dag_id='Denis_Chinese_S3_to_CH__weather',
    default_args=default_args,
    schedule_interval='0 4 * * *',  # каждый день в 04:00 UTS (CRON выражение)
    catchup=False,
    description='S3 weather to ClickHouse', # описание в заголовке после названия дага
    tags=["spark", "transform", "weather"],
)
...
s3_to_ch = SparkSubmitOperator(
    ...
    application="/opt/airflow/scripts/transform/transform__weather_denis.py",
    ...
    env_vars={
        ...
        "S3_PATH_WEATHER": "s3a://dev/Denis_Chinese/Weather",
