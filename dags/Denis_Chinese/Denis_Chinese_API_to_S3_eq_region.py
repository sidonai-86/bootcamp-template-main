...
default_args = {"owner": "Denis_Chinese", "start_date": days_ago(7)}

dag = DAG(
    dag_id="Denis_Chinese_API_to_S3_eq_region",
    default_args=default_args,
    schedule_interval="@daily",
    catchup=True,
    description="API earthquake to S3 by region",
    tags=["eq", "s3", "airflow"],
)
...
def loading_date(starttime: str, endtime: str, manager: S3MaxDateManager):
    ...
    for dt in pd.date_range(starttime, endtime, inclusive="left"):
        ...
        filename = f"Denis_Chinese/Earthquake_by_reg/events_{load_date}.json"

        hook = S3Hook(aws_conn_id="minios3_conn")
        hook.load_string(
            string_data=json.dumps(data), key=filename, bucket_name="dev", replace=True
        )
