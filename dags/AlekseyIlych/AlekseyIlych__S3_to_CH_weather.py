...
def check_new_files(**context):
    ...
    prefix = "AlekseyIlych/API_to_S3/"
    bucket = "dev"

    last_check = Variable.get(
        "AlekseyIlych_LAST_S3_CHECK_TIME__WEATHER", default_var="2020-01-01T00:00:00"
    )
    ...
    Variable.set("AlekseyIlych_LAST_S3_CHECK_TIME__WEATHER", now_str)

    manifest_content = "\n".join(path.strip() for path in new_files if path.strip())
    manifest_key = "AlekseyIlych/API_to_S3/manifests/new_files.txt"
...
dag = DAG(
    dag_id='AlekseyIlych_S3_to_CH',
    default_args=default_args,
    schedule_interval='0 8 * * *',
    catchup=True,
    description='S3 weather to ClickHouse',
    tags=['weather', 'temp', 'snowfall', 's3']
)
...
s3_to_ch = SparkSubmitOperator(
    ...
    application="/opt/airflow/scripts/transform/transform__weather_data_AlekseyIlych.py",
    ...
    env_vars={
        ...
        "S3_PATH_WEATHER": f's3a://dev/AlekseyIlych/API_to_S3',
...
send_message_telegram = TelegramOperator(
    ...
    text=(
        "S3 -> Кол-во новых файлов Weather: "
